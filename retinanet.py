...
def resize_and_pad_image(
    image, min_side=300.0, max_side=1333.0, jitter=[256, 512], stride=128.0
):
...
def preprocess_data(sample):
    ...
    name = sample["image/filename"]
    print(sample)

    # parsed.features.feature['image/text']
    image = tf.image.decode_jpeg(sample["image/encoded"])
    ...
    # bbox = swap_xy(sample["image/objects/bbox"])
...
if __name__ == "__main__":
    ...
    image = tf.keras.layers.InputLayer(shape=[None, None, 3], name="image")
    ...
    val_dataset = tfds.load("artificial_ants/debug", split="train", data_dir="data")
